...
namespace potapova
{
  ...
  Rectangle* Rectangle::expandBoundsToPoint(const Point& point) noexcept
  {
  ...
  Rectangle* Rectangle::expandBoundsToPolygon(const Polygon& polygon) noexcept
  {
    std::accumulate(polygon.points.begin(), polygon.points.end(), this, std::bind(&Rectangle::expandBoundsToPoint, _1, _2));
    return this;
  }

  Rectangle* Rectangle::expandBoundsToPolygons(const std::deque< Polygon >& polygons) noexcept
  {
