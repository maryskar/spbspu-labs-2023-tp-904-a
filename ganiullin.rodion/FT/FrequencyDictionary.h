...
namespace ganiullin {
  ...
  FreqDict merge(const FreqDict& lhs, const FreqDict& rhs, const SubCommand getNewValue);
  FreqDict getIntersect(const FreqDict& lhs, const FreqDict& rhs, const SubCommand getNewValue);
  FreqDict getDifference(const FreqDict& lhs, const FreqDict& rhs);
  template < class T > VectorDict getSorted(const FreqDict& src, const T& predicate);
  std::ostream& print(std::ostream& out, const FreqDict& src);
  std::ostream& printRareElems(std::ostream& out, const FreqDict& src, size_t num);
  std::ostream& printCommonElems(std::ostream& out, const FreqDict& src, size_t num);
