...
namespace ganiullin {
  ...
  public:
    ...
    std::ostream& execCommand(const std::string&, const std::vector< Polygon >&,
  ...
  private:
    ...
    using VertCommand = std::ostream& (*)(const Polygons& polygons,
        const size_t vertexes, std::ostream& out);
    using StateCommand
        = std::ostream& (*)(const Polygons& polygons, std::ostream& out);
